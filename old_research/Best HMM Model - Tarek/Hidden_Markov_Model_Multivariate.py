...
def select_best_model(X, min_states=2, max_states=8):
    results = []
    models = []
    for n in range(min_states, max_states + 1):
        model = train_hmm(X, n_components=n)
...
def export_states_to_csv(df, model, X,write_csv=False, output_path='earthquake_states_output.csv'):
    states = model.predict(X)
    df_out = df.copy()
...
def plot_geographic_states(df, output_path='earthquake_geographic_states.png'):
...
def generate_summary_table(df, write_csv=False, output_path='state_summary_table.csv'):
...
def plot_transition_matrix(model,write_csv=False,output_path_csv='transition_values.csv', output_path='transition_matrix_heatmap.png'):
...
def main():
    ...
    best_model, best_n_states, results_df = select_best_model(X, min_states=3, max_states=6)
    ...
    summary=generate_summary_table(df,write_csv=True)
